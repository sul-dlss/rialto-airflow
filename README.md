...
NOTE: The connection is setup automatically in the compose.yaml file with the `AIRFLOW_CONN_GOOGLE_CLOUD_DEFAULT` env variable.  It specifies the location of the key file and the other attributes.  The service account key file on the servers is generated by puppet and vault by merging a template file with values stored in vault.  For localhost, you will need to have the JSON file for the service account and you will need to place it in your local `config` directory, with the filename: `config/sul-rialto-google-drive-key.json`
