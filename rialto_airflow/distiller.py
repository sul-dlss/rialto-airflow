...
def _jsonpath_match(rule: JsonPathRule, data) -> Optional[str | int]:
    ...
    if len(results) > 0:
        ...
        if rule.only_positive_number:
            ...
            if result is None:
                logging.debug(f'could not cast {results[0].value} to a positive number; jpath={jpath}; data={data}')
        elif rule.is_valid_year:
            result = _ensure_valid_year(result)
            if result is None:
                logging.debug(f'could not cast {results[0].value} to a non-future year; jpath={jpath}; data={data}')
