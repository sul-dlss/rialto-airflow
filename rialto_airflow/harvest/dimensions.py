    ...
    with jsonl_file.open("w") as jsonl_output:
        with get_session(snapshot.database_name).begin() as select_session:
            ...
            for author in (
                ...
                for dimensions_pub_json in orcid_publications(author.orcid):
                    count += 1
                    if limit is not None and count > limit:
                        stop = True
                        break

                    # TODO: placeholder, not sure if this is the actual json structure
...
def orcid_publications(orcid: str) -> Generator[dict, None, None]:
    for doi in dois_from_orcid(orcid):
        for pub in publications_from_dois(doi):
            yield pub
