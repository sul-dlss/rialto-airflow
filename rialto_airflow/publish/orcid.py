...
from rialto_airflow.publish.reports_database import AuthorOrcids, OrcidIntegrations  # type: ignore
        ...
        with get_session(RIALTO_REPORTS_DB_NAME).begin() as insert_session:
            ...
            for row in csv_reader:
                row_values = {
                    ...
                    "orcidid": row.orcidid,
                    "orcid_update_scope": row.orcid_update_scope,
                    "role": row.role,
...
def export_orcid_integration_stats():
    ...
    # TODO: How to handle historic data?
    with get_session(RIALTO_REPORTS_DB_NAME).begin() as insert_session:
        insert_session.connection(execution_options={"isolation_level": "SERIALIZABLE"})

        for row in orcid_stats:
            row_values = {
                "read_only_scope": orcid_stats["read_only_scope"],
                "read_write_scope": orcid_stats["read_write_scope"],
            }
            insert_session.execute(insert(OrcidIntegrations).values(**row_values))
    return orcid_stats
