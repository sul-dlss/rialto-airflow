...
def export_publications() -> int:
    ...
    with get_session(RIALTO_DB_NAME).begin() as select_session:
        ...
        stmt = (
            ...
            # TODO: ignore publications that were only harvested from openalex (for now)
            .where(
                Publication.pubmed_harvested.is_not(None)
                | Publication.dim_harvested.is_not(None)
                | Publication.wos_harvested.is_not(None)
                | Publication.sulpub_harvested.is_not(None)
            )
